...
router.post('/', auth, async (req, res) => {
  try {
    // Mark all items as freshly added
...
router.put('/:id/status', auth, async (req, res) => {
  try {
    ...
    // When order is marked served, mark all items as served
    if (status === 'served') {
...
// Update order items — smart merge: preserve served items, detect new additions
router.put('/:id', auth, async (req, res) => {
  try {
    const order = await Order.findOne({ _id: req.params.id, restaurantId: req.restaurantId });
    if (!order) return res.status(404).json({ message: 'Order not found' });
    ...
    // Smart item merge: preserve served status for existing items, flag new items
    if (update.items && Array.isArray(update.items)) {
      const existingItemsMap = {};
      order.items.forEach(item => {
        // Key by name (since menuItemId may not always be present in older items)
        existingItemsMap[item.name] = item;
      });

      update.items = update.items.map(newItem => {
        const existing = existingItemsMap[newItem.name];
        if (existing) {
          // Item existed before — preserve its served state
          return {
            ...newItem,
            served: existing.served,
            addedAt: existing.addedAt || order.createdAt,
            servedAt: existing.servedAt || null
          };
        } else {
          // Brand new item added in this update — mark as freshly added
          return {
            ...newItem,
            served: false,
            addedAt: new Date(),
      ...
      // Record when waiter last updated the order so kitchen can see "updated X mins ago"
      update.lastUpdatedByWaiter = new Date();
    }
